...
$scriptPath = "<INSTALL_DIR>\UnicodeReplacementTool\unicode_replacer.py"
if (Test-Path $scriptPath) {
    Write-Host "[OK] Unicode replacer script found" -ForegroundColor Green
}
else {
    Write-Host "[FAIL] Unicode replacer script not found at: $scriptPath" -ForegroundColor Red
    exit 1
}
...
Write-Host "[OK] Skill installed to: $targetPath" -ForegroundColor Green
Write-Host ""
...
if (!(Test-Path $skillScriptLink)) {
    try {
        ...
        Write-Host "[OK] Python script linked to skill directory" -ForegroundColor Green
    }
    catch {
        Write-Host "[WARNING] Could not link Python script (non-critical)" -ForegroundColor Yellow
    }
}
